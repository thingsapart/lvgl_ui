    ...
    switch (expr->base.type) {
        ...
        case IR_EXPR_LITERAL: {
            ...
            if (lit->is_string) {
                ...
                if (v && strncmp(v, "S:~/", 4) == 0) {
                    const char* rest = v + 3; /* starts with '/' */
                    printf("lvgl_ui_expand_home_path(");
                    print_c_string_literal(rest, strlen(rest));
                    printf(")");
                } else if (v && strncmp(v, "S:/", 3) == 0 && home) {
                    const char* after_s = v + 2; /* points at absolute path */
                    size_t homelen = strlen(home);
                    if (strncmp(after_s, home, homelen) == 0) {
                        const char* rest = after_s + homelen;
                        printf("lvgl_ui_expand_home_path(");
                        print_c_string_literal(rest, strlen(rest));
                        printf(")");
                    } else {
    ...
    for (IRObject* current = head; current; current = current->next) {
        ...
        if (strcmp(current->c_type, "const char*") == 0) {
            ...
            if (current->constructor_expr && current->constructor_expr->base.type == IR_EXPR_FUNCTION_CALL) {
                IRExprFunctionCall* fcall = (IRExprFunctionCall*)current->constructor_expr;
                if (strcmp(fcall->func_name, "malloc") == 0) {
                    print_indent(content_indent);
                    printf("if (%s == NULL) {\n", current->c_name);
                    print_indent(content_indent);
                    printf("    lvgl_ui_handle_oom(\"%s\");\n", current->c_name);
                    print_indent(content_indent);
                    printf("}\n");
                }
            }
...
void c_code_print_backend(IRRoot* root, const ApiSpec* api_spec) {
    ...
    printf("#include <stdio.h>  // For snprintf\n");
    printf("#include <string.h> // For strlen/getenv handling\n\n");
    ...
    /* Helper: Out-of-memory handler used by generated code when allocations fail.
     * Define `LVGL_UI_OOM_FOREVER` to make the handler spin forever instead of
     * intentionally faulting the CPU. This is useful when running under an
     * RTOS where a crash would be undesirable. */
    printf("// --- OOM / Home-path helpers ---\n");
    printf("static void lvgl_ui_handle_oom(const char* where) {\n");
    printf("    LV_LOG_ERROR(\"LVGL UI: OOM at %%s\", where);\n");
    printf("#ifdef LVGL_UI_OOM_FOREVER\n");
    printf("    for(;;) { /* spin */ }\n");
    printf("#else\n");
    printf("    volatile int* _p = (volatile int*)0; *_p = 0; /* force crash */\n");
    printf("#endif\n");
    printf("}\n\n");

    printf("static char* lvgl_ui_expand_home_path(const char* rest) {\n");
    printf("    const char* _h = getenv(\"HOME\");\n");
    printf("    if (_h) { size_t _n = strlen(_h) + strlen(rest) + 2; char* _buf = LVGL_UI_MALLOC(_n); if (!_buf) { lvgl_ui_handle_oom(\"expand_home_path\"); return NULL; } snprintf(_buf, _n, \"%%s%%s\", _h, rest); return _buf; }\n");
    printf("    return (char*)rest; /* fallback: return literal rest when HOME unavailable */\n");
    printf("}\n\n");
