    ...
    if (cJSON_IsString(value)) {
        ...
        if (s[0] == '@') {
            ...
            if (registered_c_var) {
                ...
                if (strstr(s + 1, "style") != NULL || strstr(registered_c_var, "style") != NULL ) {
...
static void process_properties(GenContext* ctx, cJSON* props_json_input, const char* target_c_var_name, IRStmtBlock* current_block, const char* obj_type_for_api_lookup, cJSON* ui_context) {
    if (!props_json_input) return;

    // Determine if properties are direct children or under a "properties" key
    cJSON* props_json = props_json_input;
    if (cJSON_GetObjectItem(props_json_input, "properties") && strcmp(obj_type_for_api_lookup, "style") != 0) {
        // For widgets/components, properties are under "properties".
        // For styles, they might be direct children of the style node if no "properties" field exists.
        props_json = cJSON_GetObjectItem(props_json_input, "properties");
        if (!props_json) return; // No properties object found for this widget/component
    } else if (strcmp(obj_type_for_api_lookup, "style") == 0) {
        // For styles, if there's no "properties" field, the props_json_input *is* the properties object.
        // If there IS a "properties" field, use that.
        cJSON* style_props_sub_object = cJSON_GetObjectItem(props_json_input, "properties");
        if (style_props_sub_object) props_json = style_props_sub_object;
    }
    ...
    for (prop = props_json->child; prop != NULL; prop = prop->next) {
        ...
        if (!prop_name) continue; // Should not happen for named object items

        // Skip "type", "id", "named", "context", "children", "view_id", "inherits", "create", "c_type", "init_func"
        // if they are accidentally passed in props_json (e.g. when node_json itself is passed for styles)
        ...
        if (!prop_def) {
            if (strcmp(obj_type_for_api_lookup, "obj") != 0 && strcmp(obj_type_for_api_lookup, "style") != 0) { // Don't try "obj" if already "obj" or "style"
        ...
        const char* part_str = prop_def->style_part_default ? prop_def->style_part_default : "LV_PART_MAIN"; // Provide default if NULL
...
static void process_node(GenContext* ctx, cJSON* node_json, IRStmtBlock* parent_block, const char* parent_c_var, const char* default_obj_type, cJSON* ui_context) {
    ...
    // For component instances (e.g. type_str = "@my_comp"), widget_def will be NULL here.
    // This is handled inside the 'else' (widget processing) block.
    ...
    if (strcmp(type_str, "use-view") == 0) {
        ...
        if (!cJSON_IsString(component_ref_item) || !component_ref_item->valuestring || component_ref_item->valuestring[0] == '\0') {
            ...
            if (component_def_id_from_json[0] != '@') {
                ...
                if (!component_root_json) {
                    ...
                    // The c_var_name_for_node of the use-view itself is not directly used to create an object for use-view.
                    // Instead, the component's root object will be created.
                    // If use-view had a "named" attribute, it's ambiguous what it names.
                    // We pass parent_c_var and current_node_ir_block to the component's root.
        ...
        // widget_c_type_name for ir_new_widget_allocate_stmt is usually "lv_obj_t" or what widget_def->c_type would be if it were for widgets.
        // For now, hardcoding "lv_obj_t" as it's the common return type for create functions.
        ...
        cJSON* props_json_widget = cJSON_GetObjectItem(node_json, "properties");
        process_properties(ctx, props_json_widget, c_var_name_for_node, current_node_ir_block, type_str, effective_context);
        ...
        // --- Object (e.g. style, anim) with init_func Path ---
        if (!widget_def->c_type || widget_def->c_type[0] == '\0') {
            ...
            cJSON* props_json_object = cJSON_GetObjectItem(node_json, "properties");
             if (!props_json_object && cJSON_IsObject(node_json) && strcmp(type_str,"style")==0) { // Styles might have props at top level
                props_json_object = node_json;
            }
            process_properties(ctx, props_json_object, c_var_name_for_node, current_node_ir_block, type_str, effective_context);
        }
    } else if (type_str[0] == '@') { // Component Instance without direct widget_def (should be resolved to one above ideally)
        ...
        if (!component_root_json) {
            ...
            // This is a component instance. Process its root node.
            // The c_var_name_for_node (if from "named" or "id" on the instance) applies to the instance's root object.
            // This requires process_node to potentially accept an override for the C var name of its root element.
            // For now, the component's root will generate its own name if it has "named" or "id".
            // This c_var_name_for_node for the component instance itself is not directly used to create a C var.
            // We pass parent_c_var along.
            // IR for this component instance goes into current_node_ir_block.
            process_node(ctx, (cJSON*)component_root_json, current_node_ir_block, parent_c_var, NULL, effective_context);
        }
    } else {
    ...
    if (obj_expr->type == IR_EXPR_VARIABLE) {
        ...
        if (obj_expr->type == IR_EXPR_ADDRESS_OF) {
            ...
            if (addr_of->expr && addr_of->expr->type == IR_EXPR_VARIABLE) {
                if (strstr(((IRExprVariable*)addr_of->expr)->name, "style") != NULL) { // TODO: Better type inference
                    temp_var_c_type = "lv_style_t*"; // Assume pointer type for styles
                    obj_type_for_props = "style";
                }
            }
        }
...
static void process_styles(GenContext* ctx, cJSON* styles_json, IRStmtBlock* global_block) {
    ...
    for (style_item = styles_json->child; style_item != NULL; style_item = style_item->next) {
        ...
        char* style_c_var = sanitize_c_identifier(style_name_json); // Sanitize the JSON key
        ...
        // Use IRStmtObjectAllocate for styles defined in the global "styles" block
        ir_block_add_stmt(global_block, ir_new_object_allocate_stmt(style_c_var, "lv_style_t", "lv_style_init"));

        cJSON* props_to_process = cJSON_GetObjectItem(style_item, "properties");
        if(!props_to_process) props_to_process = style_item; // Fallback if props are direct children

        process_properties(ctx, props_to_process, style_c_var, global_block, "style", NULL);
        free(style_c_var);
    }
}
...
IRStmtBlock* generate_ir_from_ui_spec(const cJSON* ui_spec_root, const ApiSpec* api_spec) {
    ...
    // If there's a global "styles" object in the root array (e.g. {"type": "styles_definition_block", "styles": {...}})
    // This is different from individual {"type": "style"} items.
    // The current generate_ir_from_ui_spec iterates a flat array.
    // The old process_styles expected a specific cJSON object.
    // For now, we assume individual style definitions are handled by process_node if "type": "style".
    // A separate global "styles" object processing is removed as ui_spec_root is now an array of nodes.
    ...
    cJSON_ArrayForEach(item_json, ui_spec_root) {
        ...
        // All other types (widgets, styles as individual items, use-view) are processed by process_node.
        // Top-level items are parented to "parent" C variable from create_ui(lv_obj_t* parent)
