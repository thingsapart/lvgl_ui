...
# ----------------------------------------------------------------------
# ---- NEW CODE: C file generator for dynamic LVGL function calls ----
# ----------------------------------------------------------------------
...
class CCodeGenerator:
    ...
    def _get_archetype_key(self, func_info):
        ...
        key_tuple = (ret_type, target_c_type, *generalized_arg_types)
        return key_tuple
...
""")
            ...
            for i, (key, func_list) in enumerate(self.archetypes.items()):
                ...
                ret_type, target_c_type, *generalized_arg_types = key

                f.write(f"// Archetype for {len(func_list)} funcs like {ret_type} {func_list[0]['name']}({generalized_arg_types})\n")
                f.write(f"static lv_obj_t* {dispatcher_name}(generic_lvgl_func_t fn, lv_obj_t* target, cJSON* args) {{\n")

                # Get original arg types from the first function to know what parser to use
                original_args = (func_list[0]['args'][1:] if target_c_type != 'void' else func_list[0]['args'])
                ...
                if ret_type == "void":
                    f.write(f"    {call_str};\n    return NULL;\n")
                elif self._is_obj_ptr(ret_type):
                    f.write(f"    return {call_str};\n")
                else: # Other return types are ignored for now
